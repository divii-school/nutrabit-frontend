...
export default class RawService {
    ...
    async getRawList(status, page, limit, material_name_ko, category_id, sortBy, sortOrder, searchdate, to_date) {
        return await axios.post(`/admin/product_raw_material/`, { status: status,page: page, limit: limit, searchData: material_name_ko, category_id: category_id, sortBy: sortBy,sortOrder: sortOrder,from_date: searchdate,to_date: to_date }).then((res) => res.data.data.productRawMaterialData);
    }
    async viewRaw(ids) {
